				    ...
				    gpointer user_data)
{
	...
	MSU_LOG_INFO("UDN %s", udn);
	MSU_LOG_INFO("IP Address %s", ip_address);

	device = g_hash_table_lookup(upnp->server_udn_map, udn);

	if (!device) {
		MSU_LOG_INFO("Device not found. Adding");
				      ...
				      gpointer user_data)
{
	...
	if (i < device->contexts->len) {
		...
		if (device->contexts->len == 0) {
			MSU_LOG_INFO("Last Context lost. Delete device");
...
GVariant *msu_upnp_get_server_ids(msu_upnp_t *upnp)
{
	...
	while (g_hash_table_iter_next(&iter, NULL, &value)) {
		device = value;
		MSU_LOG_INFO("Have device %s", device->path);
			   ...
			   void *user_data)
{
	...
	MSU_LOG_INFO("Path: %s", task->path);
	MSU_LOG_INFO("Start: %u", task->ut.get_children.start);
	MSU_LOG_INFO("Count: %u", task->ut.get_children.count);
	...
	MSU_LOG_INFO("Filter Mask 0x%x", cb_task_data->filter_mask);
	...
	MSU_LOG_INFO("Sort By %s", sort_by);
...
on_error:
	...
	MSU_LOG_INFO("%s exit with %s", __func__,
		     !cb_data->action ? "FAIL" : "SUCCESS");
}
			    ...
			    void *user_data)
{
	...
	MSU_LOG_INFO("Path: %s", task->path);
	MSU_LOG_INFO("Interface %s", task->ut.get_prop.interface_name);
	...
	MSU_LOG_INFO("Root Object = %d", root_object);
		       ...
		       void *user_data)
{
	...
	MSU_LOG_INFO("Path: %s", task->path);
	MSU_LOG_INFO("Interface %s", task->ut.get_prop.interface_name);
	MSU_LOG_INFO("Prop.%s", task->ut.get_prop.prop_name);
		     ...
		     void *user_data)
{
	...
	MSU_LOG_INFO("Path: %s", task->path);
	MSU_LOG_INFO("Query: %s", task->ut.search.query);
	MSU_LOG_INFO("Start: %u", task->ut.search.start);
	MSU_LOG_INFO("Count: %u", task->ut.search.count);
	...
	MSU_LOG_INFO("UPnP Query %s", upnp_query);
			   ...
			   void *user_data)
{
	...
	MSU_LOG_INFO("Protocol Info: %s ", task->ut.resource.protocol_info);
	...
	MSU_LOG_INFO("Root Path %s Id %s", root_path, cb_data->id);
